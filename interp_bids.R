...
#     FROM read_parquet('CAISO_dam_big.parquet')
...
# get all ids
resource_ids <- tbl(con, "read_parquet('CAISO_dam_big.parquet')") |>
  distinct(RESOURCEBID_SEQ) |>
  arrange(RESOURCEBID_SEQ) |>
  collect() |>
  pull(RESOURCEBID_SEQ)

# unlink("interp_dataset", recursive = TRUE)
# dir.create("interp_dataset")
...
#     } else {
#       outfile <- file.path("interp_dataset", paste0("id_", id, ".parquet"))
...
# # open_dataset("interp_dataset")
# interp_tbl <- open_dataset("interp_dataset")
