...
get_mean_bid_curve <- function(RESOURCE_ID) {
  tbl_pointer |>
    ...
    group_by(group, SCH_BID_XAXISDATA) |>
    summarise(
      avg_price = mean(SCH_BID_Y1AXISDATA, na.rm = TRUE),
      num_bids = n(),
    ...
    ungroup() |>
    mutate(
      percent_total = num_bids / sum(num_bids),
...
test |>
  ggplot(aes(x = SCH_BID_XAXISDATA, y = avg_price, group = group)) +
  geom_ribbon(
    aes(
      x = SCH_BID_XAXISDATA,
      y = avg_price,
  ...
  facet_wrap(
    ~group,
    nrow = 2,
    labeller = as_labeller(c(`12-20` = "12PM-8PM", `else` = "8PM-12PM"))
  )
...
plot_curves_to_right <- function(market_data, resource_id) {
  ...
  plot_data <- market_data |>
    mutate(
      STARTDATETIME = as.factor(STARTTIME),
      STARTTIME = hms::as_hms(STARTTIME)
    ) |>
    filter(RESOURCEBID_SEQ == resource_id, !is.na(SCH_BID_XAXISDATA)) |>
    arrange(STARTDATETIME, SCH_BID_XAXISDATA) |>
    group_by(STARTTIME) |>
    arrange(SCH_BID_XAXISDATA, .by_group = TRUE) |>
    mutate(
      xend = lag(SCH_BID_XAXISDATA),
